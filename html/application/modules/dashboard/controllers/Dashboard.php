		...
		if (!$this->authentication->localdata) {
			...
			if ((int)$this->authentication->localdata['account_role'] > 0) {
				...
				$collectData['collect']['users'] = array(
					...
					'undeleted' => $this->mod_account->get_local_users('undeleted'),
				);
				$collectData['collect']['cryptocurrency'] = array(
					'marketplaces'			=> $this->mod_currency->get_marketplace(),
					'available_tickers'		=> $this->mod_currency->get_allmarketplace_tickers(),
					'enabled_tickers'		=> $this->mod_ticker->get_ticker_comparison_count_by('', FALSE),
				);
